...
public:
   ...
   /* see also get_filename() and dump_with_types - given in cool-tree.handcode.h */
   	void checkAttrRedefine(Environment*);
   	bool checkMethodRedefine(Environment*);
   	void checkFormalRedefine(Environment*);
	virtual Symbol tc(Environment *env) = 0;
#ifdef Class__EXTRAS
   Class__EXTRAS
#endif
};
...
public:
   ...
   //virtual Symbol get_name() = 0;
   //virtual Symbol get_type() = 0; // { return idtable.lookup_string("_no_type"); }
   //virtual bool checkScope() { CheckType(); return true; }

#ifdef Expression_EXTRAS
   Expression_EXTRAS
#endif

   virtual Symbol tc(Environment *) { return type; /* must override for each subclass of Expression_class to do type checking */ };
...
public:
   ...
   Class_ copy_Class_();
   void dump(ostream& stream, int n);
   Symbol get_name()   { return name; } /* #1 */
   Symbol get_parent() { return parent; }
   Features get_features() { return features; }
   Symbol tc(Environment *env);
   void checkAttrRedefine(Environment *env);
   bool checkMethodRedefine(Environment *env);
   void checkFormalRedefine(Environment*);
...
public:
   ...
   bool checkFormalRedefine(Environment*);
   Symbol tc(Environment *env);
